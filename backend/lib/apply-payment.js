...
async function grantSubscription(payment) {
  ...
  const sub = await UserSubscription.create({
    ...
    maxListAllCategories: Number(contract?.maxListAllCategories) || 0,
    maxJobListings: Number(contract?.maxJobListings) || 0,
    maxCarListings: Number(contract?.maxCarListings) || 0,
    maxApartmentListings: Number(contract?.maxApartmentListings) || 0,
    maxProductListings: Number(contract?.maxProductListings) || 0,
    maxPremiumListings: Number(contract?.maxPremiumListings) || 0,
    startsAt: now,
    expiresAt,
    status: 'active',
