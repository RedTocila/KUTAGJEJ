...
function cleanJobRoleSegment(segment) {
...
function isPlausibleJobRole(role) {
...
function sanitizeRequiredRoles(raw) {
...
function splitRawRoleSegments(raw) {
  return String(raw ?? '')
    .split(/\s*(?:\/|,|;|\+|\||\band\b|\bdhe\b)\s*/i)
    .map((role) => cleanJobRoleSegment(role))
    .filter(Boolean);
}

function inferRequiredRolesFromTitle(title) {
...
function normalizeRequiredRolesArray(raw) {
  if (Array.isArray(raw)) return sanitizeRequiredRoles(raw);
  if (typeof raw === 'string' && raw.trim()) {
    return sanitizeRequiredRoles(splitRawRoleSegments(raw));
  }
  return [];
}
