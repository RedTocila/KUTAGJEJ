...
function CategoryEditor(props: {
  ...
  const categorySyncSig = React.useMemo(
    () => `${category.title}|${category.slug}|${listingTypesSig}|${String(category.updatedAt ?? '')}`,
    [category.title, category.slug, category.updatedAt, listingTypesSig],
  );
