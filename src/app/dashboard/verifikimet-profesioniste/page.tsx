...
export default function ProfessionalVerificationAdminPage() {
  ...
  const load = React.useCallback(async () => {
    setLoading(true);
    setError(null);
    const res = await listProfessionalVerificationRequests(filter === 'pending' ? 'pending' : 'all');
  ...
  const review = async (decision: 'approve' | 'reject') => {
    if (!selected) return;
    setActing(true);
    const res = await reviewProfessionalVerificationRequest(selected.id, decision, adminNote);
    if (res.error) setError(res.error);
    else {
      setSelected(null);
      setAdminNote('');
