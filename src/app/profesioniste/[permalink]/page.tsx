...
export default async function ProfessionalListingPage({ params, searchParams }: PageProps): Promise<React.ReactNode> {
  const { permalink } = await params;
  const id = mongoIdFromPublicListingSegment(permalink);
  if (!id) {
    const landing = await loadSeoLandingRoute('professionals', [permalink]);
  ...
  if (canonRaw) {
    ...
    if (requestedNorm !== canonNorm) {
      permanentRedirect(pathsPublicVerticalListingDetail(paths.public.professionals, canonRaw));
    }
  }
