...
export default async function MarketplaceListingPage({ params, searchParams }: PageProps): Promise<React.ReactNode> {
  const { permalink } = await params;
  const id = mongoIdFromPublicListingSegment(permalink);
  if (!id) {
    const landing = await loadSeoLandingRoute('marketplace', [permalink]);
  ...
  if (canonRaw) {
    ...
    if (requestedNorm !== canonNorm) {
      permanentRedirect(pathsPublicVerticalListingDetail(paths.public.marketplace, canonRaw));
    }
  }
