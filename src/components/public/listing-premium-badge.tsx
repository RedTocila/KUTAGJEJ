...
const SEAL_AMBER = '#fbbf24';
const CROWN_WHITE = '#ffffff';
const GLOW =
  'drop-shadow(0 0 3px rgba(251, 191, 36, 0.75)) drop-shadow(0 1px 8px rgba(245, 158, 11, 0.45))';

/**
 * Premium listing badge — amber scalloped seal with white crown and soft glow.
 * Distinct from Grow/Elite {@link ListingTrustBadge} (amber seal + yellow star).
 */
export function ListingPremiumBadge({
  size = 18,
  ...
  return (
    ...
    >
      <svg
        ...
        style={{ display: 'block' }}
      >
        <path d={SCALLOPED_SEAL_PATH} fill={SEAL_AMBER} />
      </svg>
      <Box
        aria-hidden
        sx={{
      ...
      >
        <CrownSimpleIcon size={iconSize} weight="fill" color={CROWN_WHITE} />
      </Box>
    </Box>
  );
}

/** Same silhouette as ListingTrustBadge — shared seal shape, different icon. */
